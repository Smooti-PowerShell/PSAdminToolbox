	...
	process {
		foreach ($c in $ComputerName) {
			try {
				...
				$obj.psobject.typenames.insert(0, "PSAdminToolbox.OSInfo")
				Write-Output $obj
			}
			Catch {
				if ($LogErrors) {
		...
		foreach ($c in $ComputerName) {
			Try {
				...
				foreach ($disk in $disks) {
					...
					$obj.psobject.typenames.insert(0, "PSAdminToolbox.DiskInfo")
					Write-Output $obj
				}
			}
			Catch {
		...
		foreach ($c in $ComputerName) {
			try {
				...
				foreach ($processor in $processors) {
					...
					$obj = New-Object -TypeName PSObject -Property $props
					$obj.psobject.typenames.insert(0, "PSAdminToolbox.ProcessorInfo")
					Write-Output $obj
				}
			}
			Catch {
		...
		foreach ($c in $ComputerName) {
			try {
				...
				foreach ($graphicsCard in $graphicsCards) {
					...
					$obj = New-Object -TypeName PSObject -Property $props
					$obj.psobject.typenames.insert(0, "PSAdminToolbox.GraphicsCardInfo")
					Write-Output $obj
				}
			}
			Catch {
		...
		foreach ($c in $ComputerName) {
			try {
				...
				foreach ($physicalMemory in $physicalMemorys) {
					...
					$obj = New-Object -TypeName PSObject -Property $props
					$obj.psobject.typenames.insert(0, "PSAdminToolbox.PhysicalMemoryInfo")
					Write-Output $obj
				}
			}
			Catch {
